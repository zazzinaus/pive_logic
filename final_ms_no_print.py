...
def print_gpu_memory_usage():
    """Print the current and peak GPU memory usage."""
    if torch.cuda.is_available():
        gpu_memory_used = torch.cuda.memory_allocated() / (1024 ** 3)  # Convert bytes to GB
        gpu_memory_max = torch.cuda.max_memory_allocated() / (1024 ** 3)  # Convert bytes to GB
...
#         self.tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3.1-8B-Instruct") # Instruct
...
#         print("GPU memory usage after lora:")
#         print_gpu_memory_usage()
...
class TranslatorEngine:
    def __init__(self, dataset, output_filename, batch_size, sample_limit=None, start_index=0, end_index=-1, continuous=True):
        ...
        # Print GPU memory usage after applying LoRA
        print("GPU memory usage after LoRA:")
        self.print_gpu_memory_usage()
...
if __name__ == '__main__':
    # dataset = 'chunk1.json'
    # output = 'chunk1_ms_marco_out.json'
    # batch_size = 1
    # sample_limit = -1  # Process all samples, or set this to limit it to a number of samples
    # continuous = True
